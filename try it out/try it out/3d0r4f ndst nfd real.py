...
def generate_travel_plan(destination, num_days, budget, interest):
    ...
    for day in range(num_days):
        ...
        daily_destinations = affordable_places.iloc[day * 3: (day + 1) * 3]  # Select 3 destinations for each day
        if len(daily_destinations) == 0:
            travel_plan += "Sorry, we are out of places for this day.\n"
            continue
        for _, row in daily_destinations.iterrows():
    ...
    if places_available < num_days * 3:
        days_without_places = (num_days * 3) - places_available
